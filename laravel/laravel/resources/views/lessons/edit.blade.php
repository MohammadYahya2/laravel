    ...
    <div class="container mt-4">
        <div class="card">
            <div class="card-header bg-primary text-white">
                <h3 class="mb-0">Edit Lesson: {{ $lesson->title }}</h3>
            </div>
            <div class="card-body">
                @if ($errors->any())
                ...
                <form action="{{ route('lessons.update', $lesson) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content" rows="5" required>{{ old('content', $lesson->content) }}</textarea>
                        @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
